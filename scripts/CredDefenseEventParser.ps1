function Invoke-CredDefenseEventParser{
    ...
    ### Note that a honeytoken account needs to be created in the domain prior to this functionality working.
    ### After account creation a Service Principal Name needs to be registered in the Domain
    ### At a minimum this can be done after account creation with: Set-ADUser honeytoken -ServicePrincipalNames @{Add="MSSQLSvc/server161:1433"}
    ### IMPORTANT: Make sure you do not duplicate a valid SPN
    ### Also note that simply creating an account and registering an SPN does not take into account an attacker that does additional recon to determine if the account is fake (Invoke-HoneypotBuster)

    Write-Output "[*] Now checking logs for potential Kerberoasting activity`n"
    ### Change the honeytoken account below to match your fake account
    $haccount = "honeytoken"
     ...
     ForEach ($event in $events) 
     {
        $eventXML = [xml]$event.ToXml()
                if ($event.id -eq 4769)
                {
                  $acct = $eventXML.Event.EventData.Data[2]."#text"
                  $ip = $eventXML.Event.EventData.Data[6]."#text"
                  if ($acct -eq $haccount){
                  Write-host -ForegroundColor yellow "[***] The $acct honeyaccount has been requested from $ip - possible kerberoasting."
                }
